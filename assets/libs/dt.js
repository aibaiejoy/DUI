!(function($) {
	...
	$.extend($, {
		...
		where: function(data) {
			...
			try {
				...
				if(this.config.orderRule && this.config.method == "search") {
					var shownum=(this.config.shownum)?this.config.shownum:page.shownum;
					var startPoint = startPoint = ((page.nowpage - 1) * shownum- 1) >= 0 ? (page.nowpage - 1) * shownum- 1 : 0;
					orderRule = {
						orderRule: {
							"property": this.config.orderRule.property,
							"order": this.config.orderRule.order,
							"limitRule": {
								"limit": startPoint + "," + shownum
							}
						}
					}
				}
		...
		request: function(sentdata, args) {
			//对开发人员传递过来的data进行解析
			try {
			...
			if(this.config.method == "insert" || this.config.method == "delete" || this.config.method == "update" || this.config.method == "search")
...
//						alert("Request was unsuccessful: " + );
		...
		insert: function(args) {
			this.parsing(args.config); 
			this.config.method = "insert"
			this.request(JSON.stringify($.joining(args.data)),args);
		},
		delete: function(args) {
			this.parsing(args.config);
			this.config.method = "delete"
			this.request(JSON.stringify($.where(args.data)), args);
		},
		...
		upload: function(file, sc, er) {
			...
			$uploadFile.before();
			$frmDom.append($cloneFile.attr("name", "file"));
			// $("body").append($cloneFile);
			this.config = {
				method: "upload",
				server: {
